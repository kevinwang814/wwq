...
class Core_View
{
     ...
     * @var string
     */
    protected $_parser_name = 'Core_View_Parser';
     ...
     * 返回头部信息
     */
    protected $_response_header = true;

    /**
     * 字符编码
     */
    protected $_response_charset = 'utf-8';
    ...
    public static function getInstance() {
        if (!(self::$_instance instanceof self)) {
            $config['_view_dir'] = APP_PATH . '/view/';
     ...
     * @param array $config
     */
    function display($viewname, array $vars = null)
    {
        if ($this->_response_header)
        {
            header('Content-Type: text/html; charset=' . $this->_response_charset);
        }

        $view_dir = $this->_view_dir;
        if(!strpos($viewname, '.php'))
        {
            $viewname = $viewname . '.tpl.php';
        }
...
/**
 * Core_View_Parser 类实现了视图的分析
 *
 */

class Core_View_Parser
{
    /**
     * 视图文件扩展名
     *
    ...
    protected function _element($element_name, array $vars = null)
    {
        ...
        $filename = "{$this->_view_dir}/_element/{$element_name}.{$this->_extname}";
