<?php
/**
 * Filesys 类提供了一组简化文件系统操作的方法
 */
abstract class Ext_Filesys
{
    /**
    ...
    static function mkdirs($dir, $mode = 0777)
    {
        if (!is_dir($dir))
        {
            ...
            if (!$ret)
            {
                throw new Core_Exception("创建目录失败".$dir);
            }
        }
        return true;
    }
    ...
    static function rmdirs($dir)
    {
        ...
        {
            // 禁止删除根目录
            throw new Core_Exception("禁止删除根目录:".$dir);
        }
    ...
    /**
     * 递归查找文件，用于 {@link FileSys::findFiles()}
     */
